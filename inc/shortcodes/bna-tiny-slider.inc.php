...
/*
	...
	example : [bnacase-slider casecount="1" imageset="1" addtags="true" casebtn="true"]
*/

if(!is_admin()) { // dont run in Guttenberg Admin 
	add_shortcode('bnacase-slider',function( $atts , $content = null ){
		...
		*/
		...
		foreach( $casesResults as $caseResult ):
			...
			<div class="bnacase-imgset bncacase-<?=$caseResult['case_id'];?>">
				<div class="before">
					<a href="<?=$makeCaseLink; ?>"><img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="<?=$rmg_case::get_image($imgsStack['before_image_path'], 'small'); ?>" alt="bna-sample"  class="alignnone tns-lazy size-full" /></a>
					<?php
					/**
				...
				<div class="after">
					<a href="<?=$makeCaseLink; ?>"><img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="<?=$rmg_case::get_image($imgsStack['after_image_path'], 'small'); ?>" alt="bna-sample"  class="alignnone tns-lazy size-full" /></a>
					<?php
					/**
					* Include tags : addtags="true" or addtags="false"
					*/
